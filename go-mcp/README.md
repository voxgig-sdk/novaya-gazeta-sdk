...
[MCP](https://modelcontextprotocol.io) server exposing the NovayaGazeta SDK as
two agent tools — `novaya-gazeta_list` and `novaya-gazeta_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/novaya-gazeta-mcp)
make build

# 2. Provide credentials via the environment
export NOVAYA_GAZETA_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user novaya-gazeta \
  -- /absolute/path/to/novaya-gazeta-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./novaya-gazeta-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// novaya-gazeta_list: first page of records
{ "entity": "article" }
{ "entity": "article", "query": { } }

// novaya-gazeta_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/novaya-gazeta-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user novaya-gazeta \
     -- "$PWD"/dist/*/novaya-gazeta-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `novaya-gazeta_list` and `novaya-gazeta_load` tools now appear
   in new sessions. Ask the agent to *"list article using novaya-gazeta"*
   and it calls `novaya-gazeta_list` with `{"entity":"article"}`.
...
```sh
export NOVAYA_GAZETA_APIKEY=sk_live_xxx            # API key
export NOVAYA_GAZETA_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `novaya-gazeta_list` tool
...
### Call the `novaya-gazeta_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `novaya-gazeta_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `novaya-gazeta_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `NOVAYA_GAZETA_APIKEY` | API key sent with every request. |
| `NOVAYA_GAZETA_BASE` | Optional override of the API base URL. |
...
```sh
./novaya-gazeta-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"novaya-gazeta_load","arguments":{"entity":"article","query":{"id":1}}}}'
```
