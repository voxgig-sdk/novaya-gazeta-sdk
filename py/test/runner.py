...
class NovayaGazetaTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            NovayaGazetaTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = NovayaGazetaTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = NovayaGazetaTestRunner.getenv("NOVAYA_GAZETA_TEST_LIVE")
        override = NovayaGazetaTestRunner.getenv("NOVAYA_GAZETA_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = NovayaGazetaTestRunner.getenv(key)
        ...
        explain = NovayaGazetaTestRunner.getenv("NOVAYA_GAZETA_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if NovayaGazetaTestRunner._test_control is not None:
            return NovayaGazetaTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                NovayaGazetaTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            NovayaGazetaTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = NovayaGazetaTestRunner.load_test_control()
...
def load_env_local():
    NovayaGazetaTestRunner.load_env_local()


def env_override(m):
    return NovayaGazetaTestRunner.env_override(m)


def entity_data(v):
    return NovayaGazetaTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return NovayaGazetaTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return NovayaGazetaTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return NovayaGazetaTestRunner.load_test_control()


def live_client_options():
    return NovayaGazetaTestRunner.live_client_options()


def live_delay_ms():
    return NovayaGazetaTestRunner.live_delay_ms()
