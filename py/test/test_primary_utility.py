...
from novayagazeta_sdk.core.spec import NovayaGazetaSpec
from novayagazeta_sdk.core.result import NovayaGazetaResult
from novayagazeta_sdk.core.response import NovayaGazetaResponse
from novayagazeta_sdk.core.operation import NovayaGazetaOperation
...
def _err_from_map(m):
    ...
    if msg == "":
        return None
    code = m.get("code", "")
    return NovayaGazetaError(code, msg)
...
class TestPrimaryUtility:
    ...
    def test_exists(self):
        client = NovayaGazetaSDK.test(None, None)
    ...
    def test_feature_init_basic(self):
        init_client = NovayaGazetaSDK.test(None, None)
        ...
        class TestInitFeature(NovayaGazetaBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "initfeat"
                self.active = True
    ...
    def test_feature_init_inactive(self):
        ...
        class TestInitFeatureInactive(NovayaGazetaBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "nofeat"
                self.active = False
    ...
    def test_fetcher_blocked_test_mode(self):
        ...
        blocked_client = NovayaGazetaSDK({
            "base": "http://localhost:8080",
            "system": {
                "fetch": mock_fetch,
            },
    ...
    def test_make_result_no_result(self):
        ...
        utility = client._utility
        ctx = _make_test_full_ctx(client, utility)
        ctx.spec = NovayaGazetaSpec({"step": "start"})
        ctx.result = None
    ...
    def test_make_spec_basic(self):
        ...
        spec_client = NovayaGazetaSDK.test(None, setup_opts)
    ...
    def test_operator_basic(self):
        ...
        def subject(entry):
            ...
            if not isinstance(in_val, dict):
                in_val = {}
            op = NovayaGazetaOperation(in_val)
            return {
                "entity": op.entity,
                "name": op.name,
                "input": op.input,
    ...
    def test_prepare_auth_basic(self):
        ...
        auth_client = NovayaGazetaSDK.test(None, setup_opts)
