...
from novayagazeta_sdk.novayagazeta_types import (
    Theme,
    ThemeListMatch,
)
...
class ThemeEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return ThemeEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Theme]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Theme().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
