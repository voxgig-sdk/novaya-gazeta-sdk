// Command NovayaGazeta-cli is a CLI + REPL for the NovayaGazeta SDK,
...
const prompt = "novaya-gazeta"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "article theme"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      NOVAYA_GAZETA_APIKEY (api key), NOVAYA_GAZETA_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: NOVAYA_GAZETA_APIKEY carries the API key and
	// NOVAYA_GAZETA_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("NOVAYA_GAZETA_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("NOVAYA_GAZETA_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewNovayaGazetaSDK(opts)
