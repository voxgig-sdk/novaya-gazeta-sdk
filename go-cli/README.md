...
boru-driven command-line client **and** interactive REPL for the NovayaGazeta
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/novaya-gazeta-cli)
make build

# 2. See usage (words, entities, env vars)
./novaya-gazeta-cli --help

# 3. Provide credentials once, via the environment
export NOVAYA_GAZETA_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./novaya-gazeta-cli list article
./novaya-gazeta-cli list theme

# 5. Override the API base URL for a single call
NOVAYA_GAZETA_BASE=https://api.example.com ./novaya-gazeta-cli list article

# 6. No arguments -> interactive REPL
./novaya-gazeta-cli
novaya-gazeta> list article
novaya-gazeta> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/novaya-gazeta-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/novaya-gazeta-cli list article
   ```
...
Run with no arguments to open a REPL (prompt `novaya-gazeta>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/novaya-gazeta-cli`. |
...
(`./novaya-gazeta-cli <expr>`) and interactively (the REPL), and expressions compose the
