...
local ThemeEntity = {}
ThemeEntity.__index = ThemeEntity


function ThemeEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ThemeEntity)
  self._name = "theme"
  self._client = client
...
function ThemeEntity:get_name()
  return self._name
end


function ThemeEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ThemeEntity.new(self._client, opts)
end
...
function ThemeEntity:mark_deleted()
  self._deleted = true
end


function ThemeEntity:deleted()
  return true == self._deleted
end


function ThemeEntity:data_set(args)
  if args ~= nil then
...
function ThemeEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ThemeEntity:match_set(args)
  if args ~= nil then
...
function ThemeEntity:match_get()
...
function ThemeEntity:stream(action, args, callopts)
...
---@param reqmatch ThemeListMatch
---@param ctrl? table
---@return Theme[]
---@return string? err
function ThemeEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function ThemeEntity:_run_op(ctx, post_done)
  local utility = self._utility
