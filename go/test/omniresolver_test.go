...
import (
	...
	omni "github.com/voxgig-sdk/novaya-gazeta-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.NovayaGazetaError); is {
				out := map[string]any{
					"name":    "NovayaGazetaError",
...
func omniCtx(arg any, client *sdk.NovayaGazetaSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.NovayaGazetaSDK); is {
			client = live
