...
const __1 = require("..");
const SDK_NAME = 'NovayaGazetaSDK';
...
const TEST_SEED = { "entity": { "article": { "test01": { "id": "test01" } }, "theme": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+NovayaGazetaSDK\b/.test(code) ||
        /\bNovayaGazetaSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+NovayaGazetaSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/NovayaGazetaSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
