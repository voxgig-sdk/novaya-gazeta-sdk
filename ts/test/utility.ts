...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.NOVAYAGAZETA_TEST_LIVE ||
    'TRUE' === process.env.NOVAYAGAZETA_TEST_OVERRIDE
  ) {
  ...
  m.NOVAYAGAZETA_TEST_EXPLAIN = process.env.NOVAYAGAZETA_TEST_EXPLAIN || m.NOVAYAGAZETA_TEST_EXPLAIN
