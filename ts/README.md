# NovayaGazeta TypeScript SDK



The TypeScript SDK for the NovayaGazeta API — a type-safe, entity-oriented client with full async/await support.
...
```ts
const articles = await client.Article().list({ slug: [] })
...
| `tester(testopts?, sdkopts?)` | `NovayaGazetaSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `NovayaGazetaSDK.test(testopts?, sdkopts?)` | `NovayaGazetaSDK` | Create a test-mode client. |
...
| `client` | `client(): NovayaGazetaSDK` | Return the parent SDK client. |
...
novaya-gazeta/
├── src/
│   ├── NovayaGazetaSDK.ts        # Main SDK class
