...
// NovayaGazeta SDK pipeline test
//
...
{
    private static function utility(): NovayaGazetaUtility
    {
        return new NovayaGazetaUtility();
    }
    ...
    private static function ctx(array $over = []): NovayaGazetaContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new NovayaGazetaContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new NovayaGazetaOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof NovayaGazetaError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new NovayaGazetaOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = NovayaGazetaMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new NovayaGazetaOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = NovayaGazetaMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new NovayaGazetaOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = NovayaGazetaMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new NovayaGazetaSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = NovayaGazetaMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new NovayaGazetaResponse([]),
            'result' => new NovayaGazetaResult([]),
        ]);
        [, $err] = NovayaGazetaMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new NovayaGazetaSpec([]),
            'response' => null,
            'result' => new NovayaGazetaResult([]),
        ]);
        [, $err] = NovayaGazetaMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new NovayaGazetaSpec(['step' => 's']),
            'response' => new NovayaGazetaResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new NovayaGazetaResult([]),
        ]);
        [, $err] = NovayaGazetaMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new NovayaGazetaSpec(['step' => 's']),
            'response' => new NovayaGazetaResponse(self::resp(200, ['v' => 1])),
            'result' => new NovayaGazetaResult([]),
        ]);
        [, $err] = NovayaGazetaMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new NovayaGazetaSpec(['step' => 's']),
            'response' => new NovayaGazetaResponse(self::resp(200, ['v' => 2])),
            'result' => new NovayaGazetaResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        NovayaGazetaMakeResponse::call($ctx);
    ...
    {
        $preset = new NovayaGazetaResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new NovayaGazetaSpec([]),
            'response' => new NovayaGazetaResponse([]),
            'result' => new NovayaGazetaResult([]),
        ...
        [$got, $err] = NovayaGazetaMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new NovayaGazetaResult([])]);
        [, $err] = NovayaGazetaMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new NovayaGazetaSpec([]), 'result' => null]);
        [, $err] = NovayaGazetaMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new NovayaGazetaSpec(['step' => 's']),
            'result' => new NovayaGazetaResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new NovayaGazetaOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = NovayaGazetaMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new NovayaGazetaSpec(['step' => 's']),
            'result' => new NovayaGazetaResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new NovayaGazetaOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = NovayaGazetaMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new NovayaGazetaResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new NovayaGazetaSpec([]),
            'result' => new NovayaGazetaResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = NovayaGazetaMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = NovayaGazetaMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new NovayaGazetaError('boom', 'boom');
        $utility->fetcher = function (NovayaGazetaContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new NovayaGazetaSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = NovayaGazetaMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (NovayaGazetaContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = NovayaGazetaMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(NovayaGazetaResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        NovayaGazetaMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (NovayaGazetaContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new NovayaGazetaSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = NovayaGazetaMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new NovayaGazetaResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new NovayaGazetaSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = NovayaGazetaMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = NovayaGazetaMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = NovayaGazetaMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new NovayaGazetaResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, NovayaGazetaDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new NovayaGazetaResult(['ok' => false])]);
        $this->expectException(NovayaGazetaError::class);
        NovayaGazetaDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new NovayaGazetaResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', NovayaGazetaMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        NovayaGazetaMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new NovayaGazetaBaseFeature();
        $b = new NovayaGazetaBaseFeature();
        NovayaGazetaFeatureAdd::call($ctx, $a);
        NovayaGazetaFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): NovayaGazetaBaseFeature
    {
        $f = new NovayaGazetaBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        NovayaGazetaFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): NovayaGazetaContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new NovayaGazetaSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = NovayaGazetaPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        NovayaGazetaPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new NovayaGazetaResponse(['headers' => null]),
            'result' => new NovayaGazetaResult([]),
        ]);
        NovayaGazetaResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        NovayaGazetaResultBody::call($ctx);
