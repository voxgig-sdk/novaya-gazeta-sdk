...
class FtCtrl extends NovayaGazetaControl
{
...
class FtClient
{
    ...
    private NovayaGazetaUtility $utility;

    public function __construct(NovayaGazetaUtility $utility, array $options)
    {
    ...
    public function get_utility(): NovayaGazetaUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (NovayaGazetaContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public NovayaGazetaContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (NovayaGazetaContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = NovayaGazetaConfig::make_feature($name);
    ...
    private static function buildUrl(NovayaGazetaSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof NovayaGazetaError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new NovayaGazetaResponse($fetched);
        }
    ...
    private function fail(NovayaGazetaContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (NovayaGazetaContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
