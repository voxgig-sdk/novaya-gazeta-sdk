...
// NovayaGazeta SDK retry feature
...
class NovayaGazetaRetryFeature extends NovayaGazetaBaseFeature
{
    ...
    public function init(NovayaGazetaContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (NovayaGazetaContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(NovayaGazetaContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(NovayaGazetaContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
