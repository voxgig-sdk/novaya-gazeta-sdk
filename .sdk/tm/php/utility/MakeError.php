...
// NovayaGazeta SDK utility: make_error

require_once __DIR__ . '/../core/Operation.php';
require_once __DIR__ . '/../core/Result.php';
require_once __DIR__ . '/../core/Error.php';

class NovayaGazetaMakeError
{
    public static function call(?NovayaGazetaContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new NovayaGazetaContext([], null);
        }
        $op = $ctx->op ?? new NovayaGazetaOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new NovayaGazetaResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof NovayaGazetaError) ? $err->msg : (string)$err;
        $msg = "NovayaGazetaSDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new NovayaGazetaError('', $msg, $ctx);
        ...
        if ($err instanceof NovayaGazetaError) {
