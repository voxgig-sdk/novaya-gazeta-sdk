...
class NovayaGazetaSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = NovayaGazetaHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = NovayaGazetaHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!NovayaGazetaFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, NovayaGazetaFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return NovayaGazetaUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = NovayaGazetaHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = NovayaGazetaHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = NovayaGazetaHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new NovayaGazetaError($op . "_allow",
                "NovayaGazetaSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = NovayaGazetaHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new NovayaGazetaError("graphql_error",
                "NovayaGazetaSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    // Canonical facade: $client->Theme()->list() / ->load(["id" => ...]).
    // PHP method names are case-insensitive, so lowercase $client->theme()
    // resolves here too.
    public function Theme($data = null)
    {
        require_once __DIR__ . '/entity/theme_entity.php';
        if ($data === null) {
            if ($this->_theme === null) {
                $this->_theme = new ThemeEntity($this, null);
            }
            return $this->_theme;
        }
        return new ThemeEntity($this, $data);
    }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new NovayaGazetaSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
