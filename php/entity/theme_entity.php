...
class ThemeEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new ThemeEntity($this->_client, $opts);
    }

    /**
     * @param Theme|array $args Theme data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = NovayaGazetaHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Theme|array The current Theme data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Theme fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = NovayaGazetaHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Theme fields).
     */
    ...
    /**
     * List Theme items matching the given filter.
     *
     * @param ThemeListMatch|array|null $reqmatch Match filter (any subset
     *   of Theme fields) as an assoc-array; ThemeListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Theme[]|array A list of Theme items as assoc-arrays at
     *   the SDK boundary; throws NovayaGazetaError on failure (item-5 convention).
     */
