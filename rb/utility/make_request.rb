...
module NovayaGazetaUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = NovayaGazetaResponse.new({})
    ...
    if fetch_err
      ...
      response = NovayaGazetaResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = NovayaGazetaResponse.new(fetched)
    else
