...
module NovayaGazetaUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = NovayaGazetaContext.new({}, nil)
    end
    op = ctx.op || NovayaGazetaOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || NovayaGazetaResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(NovayaGazetaError) ? err.msg : err.to_s
    ...
    sdk_err = NovayaGazetaError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(NovayaGazetaError)
