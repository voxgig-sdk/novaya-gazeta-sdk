...
require_relative 'NovayaGazeta_types'
...
class NovayaGazetaSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = NovayaGazetaHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = NovayaGazetaHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, NovayaGazetaFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    NovayaGazetaUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = NovayaGazetaHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = NovayaGazetaHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = NovayaGazetaHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = NovayaGazetaHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = NovayaGazetaError.new(
        "graphql_error", "NovayaGazetaSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  # Canonical facade: client.Theme.list / client.Theme.load({ "id" => ... })
  def Theme(data = nil)
    require_relative 'entity/theme_entity'
    ThemeEntity.new(self, data)
  end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = NovayaGazetaSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
