...
class ThemeEntity
  ...
  def make
    opts = @_entopts.dup
    ThemeEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = NovayaGazetaHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Theme, Hash] the current Theme data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = NovayaGazetaHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Theme fields)
  def match_get
  ...
  #
  # @param reqmatch [ThemeListMatch, Hash, nil] match filter (any subset of
  #   Theme fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Theme>, Array] the matching Theme items; raises NovayaGazetaError on failure
